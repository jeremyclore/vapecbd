...
PRODUCTS = [
    ...
    {"id": 7, "name": "Манго", "cbd": "1%", "volume": "15 мл", "price": 1790, "image": "07_mango_1.jpg"},
    {"id": 8, "name": "Клубника", "cbd": "1%", "volume": "15 мл", "price": 1850, "image": "08_strawberry_1.jpg"},
    {"id": 9, "name": "Киви", "cbd": "1%", "volume": "15 мл", "price": 1720, "image": "09_kiwi_1.jpg"},
    {"id": 10, "name": "Яблоко", "cbd": "1%", "volume": "15 мл", "price": 1650, "image": "10_apple_1.jpg"},
    {"id": 11, "name": "Черника", "cbd": "1%", "volume": "15 мл", "price": 2150, "image": "11_blueberry_1.jpg"},
    {"id": 12, "name": "Арбуз", "cbd": "1%", "volume": "15 мл", "price": 1950, "image": "12_watermelon_1.jpg"},
]
...
def order():
    ...
    for item in data.get("items", []):
        ...
        total += qty * price
        body += f"- {item.get('name')} {item.get('cbd')} — {qty} шт × {price} ₽\n"

    body += f"\nИтого: {total} ₽\n"
    body += f"\nПочта для заявок: {ORDER_EMAIL}\n"

    # Пока заглушка почты: заказ сохраняется в orders.txt.
    # Когда появятся SMTP-данные, сюда подключим настоящую отправку на mail@mail.
    with open("orders.txt", "a", encoding="utf-8") as f:
        f.write("=" * 60 + "\n")
        f.write(subject + "\n")
        f.write(body + "\n")

    return jsonify({"ok": True, "message": "Заказ создан. Мы свяжемся с вами для подтверждения оплаты."})
